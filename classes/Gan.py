...
class GanBuilder:
    ...
    sample_period = 200
    latent_dim =    100
    train_dir =     './gan_images2'

    def __init__(self, X_train, Y_train, X_test, Y_test, batch_size = None, epochs = None, sample_period = None, latent_dim = None):
        X_train, X_test = X_train / 255.0 * 2 - 1, X_test / 255.0 * 2 - 1

        batch_size =    batch_size if batch_size is not None else self.batch_size
        epochs =        epochs if epochs is not None else self.epochs
        sample_period = sample_period if sample_period is not None else self.sample_period
